...
- **人类样本** (5091个账号ID): 来自以下文件：
  - `human.txt` (2783个): 主要为通过实时推文中以常用虚词为搜索词采集的账号和少部分手动采集的人类账号
  - `government.txt` (598个): 政府机构账号，通过微博政务榜单采集
  - `influencer.txt` (929个): 影响者账号，通过微博V影响力榜单采集
  - `media.txt` (781个): 媒体账号，以"新闻"为关键词搜索相关的认证账号采集

**标签分布说明**:
- 由于部分账号在数据采集时可能已被封禁、删除或设置为私密，实际成功提取特征并用于训练的样本数可能少于账号ID总数
- 实际成功提取特征的样本总数：10,085个（机器人：4,994个，人类：5,091个）
- 数据集划分：训练集8,068个（80%），测试集2,017个（20%）
...
|------|-------------------|----------------|----------|
| **人类 (类别0)** | 0.9601 | 0.9921 | 0.9758 |
| **机器人 (类别1)** | 0.9917 | 0.9580 | 0.9745 |
